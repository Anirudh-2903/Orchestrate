...
const Card = ({event,hasOrderLink,hidePrice}: CardProps) => {
  ...
  return (
    <div className="group relative flex min-h-[380px] md:min-h-[438px] w-full max-w-[400px] flex-col overflow-hidden rounded-xl bg-white shadow-md transition-all hover:shadow-lg">
      ...
      <div className='flex min-h-[230px] flex-col gap-3 md:gap-4 p-5'>
        {!hidePrice && <div className='flex gap-2'>
            <span className='p-semibold-14 rounded-full line-clamp-1 bg-green-100 px-4 py-1 text-green-60'>{event.isFree ? 'FREE' : `₹ ${event.price}`}</span>
            <p className='p-semibold-14 rounded-full line-clamp-1 bg-grey-500/10 px-4 py-1 text-grey-500'>{event.category.name}</p>
        </div>}
